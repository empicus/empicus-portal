import { Injectable, Inject } from '@angular/core';
import { Store } from '@ngrx/store';
import { Observable } from 'rxjs';
import { DataService, UrlService, RequestData, ResponseData } from 'helion-core-http';
import { PortalDiseaseContactModuleState } from "../../store/portal-disease-contact-module.state";
import { AbstractData, ErrorData } from 'helion-core';
import { FormDataUtils } from 'helion-ui-forms';
import { 
    submitDiseaseContactFormSuccessAction, 
    submitDiseaseContactFormFailureAction 
} from "../../store/actions/disease-contact-form/disease-contact-form.actions";


@Injectable({
    providedIn: 'root'
})
export class DiseaseContactFormService {

    constructor(private store: Store<PortalDiseaseContactModuleState>,
        private httpService: DataService, private urlService: UrlService) {
    
    } 

    public submit(formData) {
        let requestData = FormDataUtils.trimEntityData(formData);

        this.create(requestData, 'diseaseContactCreate').subscribe((data) => {
                this.store.dispatch(submitDiseaseContactFormSuccessAction({data: data.item}));
            },
            (error) => this.store.dispatch(submitDiseaseContactFormFailureAction({data: new ErrorData(error, requestData)}))
        );
    }

    /**
     * 
     * @param data 
     * @param serviceName 
     */
    public create<T extends AbstractData>(data: T, serviceName: string): Observable<ResponseData> {
        let request: RequestData<T> = new RequestData<T>();
        request.data = data;
        request.serviceName = serviceName;
        request.serviceURL = this.urlService.getAndromedaURL() + serviceName;
        
        return this.httpService.create(request);
    }
}

