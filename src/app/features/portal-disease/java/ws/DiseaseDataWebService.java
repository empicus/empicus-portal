 ...
 */
@Stateless
@WebServiceProvider(name = "diseaseData")
@WebServiceAllowedMethods(names = { WebConstants.REQUEST_METHOD_HTTP_GET })
public class DiseaseDataWebService extends AbstractWebService implements WebService {
	...
	protected ResponsePayloadData doExecute(RequestData requestData) throws BusinessServiceException {
		...
		String diseaseCode = webRequestData.getArtifactCode();
		Disease disease = diseaseService.findByCode(diseaseCode);
		return new ResponsePayloadData(disease.toEntityData());
	}
