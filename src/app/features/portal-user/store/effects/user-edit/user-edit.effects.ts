import { Injectable, NgZone } from '@angular/core';
import { of } from 'rxjs';
import { map, delay } from 'rxjs/operators';
import { Store } from '@ngrx/store';
import { Actions, ofType, createEffect } from '@ngrx/effects';
import { UiModuleState, navigateToAction } from 'helion-ui';
import { openUiNotificationAction, UiNotificationModuleState, UI_NOTIFICATION_TY_SUCCESS } from 'helion-ui-notifications';
import * as formActions from '../../actions/user-edit/user-edit.actions';
import { UserEditService } from '../../../services/user-edit/user-edit.service';

@Injectable()

export class UserEditEffects {

    constructor(
        private uiStore: Store<UiModuleState>,
        private userEditService: UserEditService,
        private uiNotificationStore: Store<UiNotificationModuleState>,
        private actions$: Actions, private ngZone: NgZone) { }

    /**
     * 
     */
    submitUserEditRequestEffect$ = createEffect(() => this.actions$.pipe(
        ofType(formActions.submitUserEditRequestAction),
        map(action => of(this.userEditService.submit(action.data)))),
        { dispatch: false }
    );
    
    /**
     * 
     */
    submitUserEditSuccessEffect$ = createEffect(() => this.actions$.pipe(
        ofType(formActions.submitUserEditSuccessAction),
        delay(500),
        map((action) => {
            let that = this
            this.uiNotificationStore.dispatch(openUiNotificationAction({
                data: {
                    type: UI_NOTIFICATION_TY_SUCCESS,
                    title: 'UserEdit Success',
                    message: 'userEdit information action completed',
                    okAction: () => {
                        that.uiStore.dispatch(navigateToAction({
                            data: {
                                route: '/portal-user/user-edit/view-user-edit/' + action.data.fieldData['code'].value,
                                routeData: []
                            }
                        }))
                    }
                }
            }))
            return of();
        })),
        { dispatch: false }
    );
    
}
